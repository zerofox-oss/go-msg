...
func makeOCSpanContext() (trace.SpanContext, string) {
	b := make([]byte, 24)
	rand.Read(b)
...
// is present, a OC span is started is started and set in the receive context with the correct
// parent context
func TestDecoder_SuccessfullyDecodesSpanWhenTraceContextIsPresent(t *testing.T) {
	...
	go func() {
		...
		span := oteltrace.SpanFromContext(result.ctx)
		if span == nil {
			t.Errorf("span was not expected to be nil")
		}

		receivedSC := span.SpanContext()
		ocReceivedSC := ocbridge.OTelSpanContextToOC(receivedSC)

		if ocReceivedSC.TraceID != sc.TraceID {
